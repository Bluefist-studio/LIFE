  ...
  if (!isLocked) {
    ...
    panel.querySelectorAll('.budui-block').forEach(block => {
      ...
      if (first && second && totalField) {
        ...
        let total = (parseFloat(first.value) || 0) + (parseFloat(second.value) || 0);
        totalField.value = total.toFixed(2);
        // Mark total as paid if either field is paid and filled
        const firstPaid = first.classList.contains('budui-paid') && first.value.trim() !== '';
        const secondPaid = second.classList.contains('budui-paid') && second.value.trim() !== '';
