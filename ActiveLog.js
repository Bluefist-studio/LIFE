// TEMPORARY LOGOUT BUTTON HANDLER
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", function() {
      auth.signOut().then(() => {
        ...
        store.session = { userId: null, username: null };
        save();
        showLogin();
      });
    });
...
function hideAllForms() {
  if (typeof activityForm !== 'undefined' && activityForm) activityForm.classList.add("hidden");
  if (typeof profileForm !== 'undefined' && profileForm) profileForm.classList.add("hidden");
  if (typeof screen !== 'undefined' && screen) screen.classList.remove("friends-screen");
  const friendControls = document.getElementById("friendControls");
  if (friendControls) friendControls.classList.add("hidden");

  if (typeof deleteForm !== 'undefined' && deleteForm) deleteForm.classList.add("hidden"); // keep old form hidden
}
...
function showActivityForm() {
  hideAllForms();
  if (typeof screen !== 'undefined' && screen) screen.textContent = "";
  if (typeof activityForm !== 'undefined' && activityForm) {
    activityForm.classList.remove("hidden");
    activityForm.classList.remove("editing"); // reset edit mode
  }

  if (typeof addActivityBtn !== 'undefined' && addActivityBtn) addActivityBtn.classList.remove("hidden");
  if (typeof savechangesBtn !== 'undefined' && savechangesBtn) savechangesBtn.classList.add("hidden");

  store.editingActivity = null;

  // Reset buttons
  if (typeof addActivityBtn !== 'undefined' && addActivityBtn) addActivityBtn.textContent = "Add Activity";

  if (typeof deleteActivityBtn !== 'undefined' && deleteActivityBtn) deleteActivityBtn.classList.add("hidden");

  const cancelBtn = document.getElementById("cancelEditWindowBtn");
  if (cancelBtn) cancelBtn.classList.add("hidden");

  // Reset fields
  if (typeof activityType !== 'undefined' && activityType) activityType.value = "";
  if (typeof activityDuration !== 'undefined' && activityDuration) activityDuration.value = "";
  if (typeof activityNotes !== 'undefined' && activityNotes) activityNotes.value = "";
...
if (typeof modalActivityInput !== 'undefined' && modalActivityInput) {
...
if (typeof saveProfileBtn !== 'undefined' && saveProfileBtn) {
...
auth.onAuthStateChanged(async (user) => {
  ...
  if (user) {
    ...
    const logBtn = document.querySelector('#menu button[data-action="log"]');
    if (logBtn) logBtn.classList.add('active');
    const healthBarWrapper = document.getElementById("healthBarWrapper");
    if (healthBarWrapper) healthBarWrapper.style.display = "block";
