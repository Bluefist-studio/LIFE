//saving.js
//===============================SAVINGS LOCK/UNLOCK LOGIC=============================
// Lock/unlock all editable fields in the Savings section for Budget Categories Accordion
window.toggleLockSavings = function toggleLockSavings() {
  const lockBtn = document.getElementById('lock-savings');
  if (!lockBtn) return;
  const lockIcon = document.getElementById('lock-savings-icon');
  ...
  if (!isLocked) {
    ...
    if (typeof saveSavingsToFirestore === 'function') saveSavingsToFirestore();
  } else {
    // Unlock all fields
    editFields.forEach(f => {
    ...
    panel.querySelectorAll('.budui-block').forEach(block => {
      ...
      if (first && second && totalField) {
        let total = 0;
        if (first.classList.contains('budui-paid')) total += parseFloat(first.value) || 0;
        if (second.classList.contains('budui-paid')) total += parseFloat(second.value) || 0;
        totalField.value = total.toFixed(2);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (savingsPanel) {
    ...
    savingsPanel.addEventListener('click', function(e) {
...
async function saveSavingsToFirestore() {
  ...
  // Only save up to 10 blocks for Savings
...
// Save savings to Firestore when fields change
// (handled above in DOMContentLoaded block)